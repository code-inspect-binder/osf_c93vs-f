#------------------------------------------------------------------------------#
#This file contains R-code for data analyses of Experiment 9 reported in:      #
# Wöhner, S. (2018).                                                           #
...
#dat = read.table(file = "exp09_data.txt", header = TRUE)
#dat$subj = as.factor(dat$subj)
#dat$soa = as.factor(dat$soa)

#variant 2 (choose only one)
load("exp09_prep.RData")

#variant 3 (choose only one)
#dat = read.table(file = "https://osf.io/vgqj9/download", header = TRUE)
#dat$subj = as.factor(dat$subj)
#dat$soa = as.factor(dat$soa)
...
###################################
#save workspace
save.image(file="exp09_anova.RData")
#------------------------------------------------------------------------------#
#This file contains R-code for data preparation of Experiment 11 reported in:  #
# Wöhner, S. (2018).                                                           #
...
rawdat = read.table(file = "exp11_rawdata.txt", header = TRUE)
#variant 2 (choose only one)
#rawdat = read.table(file = "https://osf.io/kzsd7/download", header = TRUE)
...
#variant 2 (choose only one)
#targlab <- read.table(file="https://osf.io/qnms9/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#variant 2 (choose only one)
#distlab <- read.table(file="https://osf.io/47tga/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#save workspace
save(list=c("dat", "rawdat"), file="exp11_prep.Rdata")

#save data
write.table(dat, "exp11_data.txt")
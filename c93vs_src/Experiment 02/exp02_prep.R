#------------------------------------------------------------------------------#
#This file contains R-code for data preparation of Experiment 2 reported in:   #
# Wöhner, S. (2018).                                                           #
...
rawdat = read.table(file = "exp02_rawdata.txt", header = TRUE)
#variant 2 (choose only one)
#rawdat = read.table(file = "https://osf.io/p5zft/download", header = TRUE)
...
#- BPR_dir = file name of task 1 stimuli (direction of masked arrows)
...
dropvars = c("BPR_Resp.x","BPR_corrResp.x", "task.soa.x","cond.x","targ_ID.x","dist_ID.x",
             ...
             "BPR_corrResp.y", "row.y", "ERR.code.old.y", "row_diff", "i", "j", "BPR_dir.x", 
             "BPR_dir.y", "BPR_pos.x", "BPR_pos.y")

rawdat = rawdat[,!(names(rawdat) %in% dropvars)]
rawdat = droplevels(rawdat)

#rename variables
names(rawdat) <- c("trial", "subj", "RT.task1", "ERR.code.task1", "RT.task2", "ERR.code.task2", "task.soa", "cond",
                   "targ_ID", "dist_ID", "blk", "ERR.task1", "ERR.task2.old", "ERR.task2")
...
#variant 2 (choose only one)
#targlab <- read.table(file="https://osf.io/2pgj9/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#variant 2 (choose only one)
#distlab <- read.table(file="https://osf.io/7ujfp/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
rawdat$dist_ID = factor(rawdat$dist_ID, levels=distlab$id, labels=distlab$label)
rawdat$blk = as.factor(rawdat$blk)

#exclude erroneous responses and RTs >=3,000/3,500 ms and RTs<=100/300 ms from latency analyses
#note: response window = 3,000 ms (task 1 at task.soa = 0 ms; task 2); 3,500 ms (task 1 at task.soa = 500 ms);
...
rawdat$RT.task1[rawdat$task.SOA==500 & rawdat$RT.task1>=3500 | rawdat$RT.task1<=100] = NA
...
#save workspace
save(list=c("dat", "rawdat"), file="exp02_prep.Rdata")

#save data
write.table(dat, "exp02_data.txt")
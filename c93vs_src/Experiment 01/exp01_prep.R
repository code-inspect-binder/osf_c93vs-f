#------------------------------------------------------------------------------#
#This file contains R-code for data preparation of Experiment 1 reported in:   #
# Wöhner, S. (2018).                                                           #
...
rawdat = read.table(file = "exp01_rawdata.txt", header = TRUE)
#variant 2 (choose only one)
#rawdat = read.table(file = "https://osf.io/gjbmf/download", header = TRUE)
...
dropvars <- c("x1", "eat", "targ", "dist", "cat.dir", "cat.corr","subset", "jitter", "sem.kat", "phon.ons", 
...
#variant 2 (choose only one)
#targlab <- read.table(file="https://osf.io/j7uqs/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#variant 2 (choose only one)
#distlab <- read.table(file="https://osf.io/fekq4/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#save workspace
save(list=c("dat", "rawdat"), file="exp01_prep.Rdata")

#save data
write.table(dat, "exp01_data.txt")
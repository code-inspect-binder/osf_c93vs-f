#------------------------------------------------------------------------------#
#This file contains R-code for data analyses of Experiment 4 reported in:      #
# Wöhner, S. (2018).                                                           #
...
#dat = read.table(file = "exp04_data.txt", header = TRUE)
#dat$subj = as.factor(dat$subj)
#dat$soa = as.factor(dat$soa)

#variant 2 (choose only one)
load("exp04_prep.RData")

#variant 3 (choose only one)
#dat = read.table(file = "https://osf.io/stvz5/download", header = TRUE)
#dat$subj = as.factor(dat$subj)
#dat$soa = as.factor(dat$soa)
...
SEM=sd(agg.subj.wide_C$PCT.semantic-agg.subj.wide_C$PCT.unrelated)/sqrt(length(levels(dat$subj)))
print(res)
print(SEM, digits=4)
...
###################################
#save workspace
save.image(file="exp04_anova.RData")
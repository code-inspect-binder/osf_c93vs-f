#------------------------------------------------------------------------------#
#This file contains R-code for data preparation of Experiment 7 reported in:   #
# Wöhner, S. (2018).                                                           #
...
rawdat = read.table(file = "exp07_rawdata.txt", header = TRUE)
#variant 2 (choose only one)
#rawdat = read.table(file = "https://osf.io/97whv/download", header = TRUE)
...
#- cond = distractor condition (1: congruent.related; 2: semantic.related; 3: semantic.unrelated; 4: congruent.unrelated)
...
rawdat$cond = factor(rawdat$cond, levels=c(1,2,3,4),labels=c("congruent.related", "semantic.related", "semantic.unrelated", "congruent.unrelated"))
rawdat$soa = as.factor(rawdat$soa)
#variant 1 (choose only one)
targlab <- read.table(file="targlab.txt", stringsAsFactors=F, header=F, col.names=c("id", "label"))
#variant 2 (choose only one)
#targlab <- read.table(file="https://osf.io/s38yq/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#variant 2 (choose only one)
#distlab <- read.table(file="https://osf.io/exu5y/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
rawdat$dist_ID = factor(rawdat$dist_ID, levels=distlab$id, labels=distlab$label)
rawdat$blk = as.factor(rawdat$blk)

#split condition: Rel (con, sem), Rtd (rel, unr)
rawdat$rel = "congruent"
rawdat$rel[rawdat$cond=="semantic.related" | rawdat$cond=="semantic.unrelated"] = "semantic"
rawdat$rel = as.factor(rawdat$rel)
rawdat$rtd = "related"
rawdat$rtd[rawdat$cond=="congruent.unrelated" | rawdat$cond=="semantic.unrelated"] = "unrelated"
rawdat$rtd = as.factor(rawdat$rtd)
names(rawdat)[names(rawdat) == 'cond'] = 'cond.old'
...
#------outlier coding------#
#M and SD per subject and item depending on distractor condition, i.e., rel x rtd, and soa (independent variables)
subj.M.SD = ddply(rawdat,.(subj,rel,rtd,soa), summarize, 
                  subj.M=mean(RT, na.rm=T), subj.SD=sd(RT, na.rm=T))
tar.M.SD = ddply(rawdat,.(targ_ID,rel,rtd,soa), summarize,
                 tar.M=mean(RT, na.rm=T), tar.SD=sd(RT, na.rm=T))
rawdat = merge(rawdat, subj.M.SD, by=c("subj", "rel", "rtd", "soa"))
rawdat = merge(rawdat, tar.M.SD, by=c("targ_ID", "rel", "rtd", "soa"))
...
dropvars = c("subj.M","subj.SD","tar.M","tar.SD","subj.min","subj.max",
             "tar.min","tar.max", "cond.old")

dat = rawdat[,!(names(rawdat) %in% dropvars)]
dat = droplevels(dat)

#save workspace
save(list=c("dat", "rawdat"), file="exp07_prep.Rdata")

#save data
write.table(dat, "exp07_data.txt")
#------------------------------------------------------------------------------#
#This file contains R-code for data preparation of Experiment 5 reported in:   #
# Wöhner, S. (2018).                                                           #
...
rawdat = read.table(file = "exp05_rawdata.txt", header = TRUE)
#variant 2 (choose only one)
#rawdat = read.table(file = "https://osf.io/9emw4/download", header = TRUE)
...
rawdat <- rawdat[rawdat$eat == "sonatst" & rawdat$cond!=4, ]
...
dropvars <- c("x1", "eat", "targ", "dist", "targ_pos","subset", "sem.kat", "phon.ons", 
...
#variant 2 (choose only one)
#targlab <- read.table(file="https://osf.io/ysw9b/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#variant 2 (choose only one)
#distlab <- read.table(file="https://osf.io/qphfa/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#save workspace
save(list=c("dat", "rawdat"), file="exp05_prep.Rdata")

#save data
write.table(dat, "exp05_data.txt")
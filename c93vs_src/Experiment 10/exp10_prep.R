#------------------------------------------------------------------------------#
#This file contains R-code for data preparation of Experiment 10 reported in:  #
# Wöhner, S. (2018).                                                           #
...
rawdat = read.table(file = "exp10_rawdata.txt", header = TRUE)
#variant 2 (choose only one)
#rawdat = read.table(file = "https://osf.io/ea34q/download", header = TRUE)
...
#- x2 = specifies orientation of masked arrows in catch trials (1: left; 2: right) or file name of target pictures superimposed by visual distractor words (leftover from Exp 09)
...
#- cross.coding(1-3) = index of sounds and corresponding words to avoid occurences on consecutive trials
#- targ_pho = phonological onset/ initial phonological units of a sound name
#- dist_pho = phonological onset/ initial phonological units of a word name
#- targ_kat = superordinate category of a target sound (e.g., bird, mammal, vehicle, ...)
...
#variant 2 (choose only one)
#targlab <- read.table(file="https://osf.io/cbz8e/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#variant 2 (choose only one)
#distlab <- read.table(file="https://osf.io/ubq7x/download", stringsAsFactors=F, header=F, col.names=c("id", "label"))
...
#save workspace
save(list=c("dat", "rawdat"), file="exp10_prep.Rdata")

#save data
write.table(dat, "exp10_data.txt")